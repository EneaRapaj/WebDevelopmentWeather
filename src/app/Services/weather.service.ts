...
export class WeatherService {
  ...
  //Variablat që do të plotësohen nga endpoint të API
  locationDetails?: LocationDetails;
  weatherDetails?: WeatherDetails;

  //Variablat që kanë të dhënat e nxjerra nga variablat e pikës fundore të API
  ...
  //variablat që do të përdoren për thirrjet API
  cityName:string = 'Mumbai';
  language:string = 'en-US';
  date:string = '20200622';
  units:string = 'm';

  //Variabla që mban kohën aktuale
  currentTime:Date;

  // variablat për të kontrolluar tabs
  today:boolean = false;
  week:boolean = true;

  //variablat për të kontrolluar metric value
  ...
  getSummaryImage(summary:string):string{
    //Adresa e dosjes bazë që përmban  images
  ...
  //Metoda për të krijuar një chunk për left container duke përdorur modelin TemperatureData
  ...
  //Metoda për të krijuar një chunk për right container duke përdorur modelin WeekData
  fillWeekData(){
    var weekCount =0;
  ...
  //Metoda për marrjen today's highlight data nga ndryshorja bazë
  fillTodaysHighlight(){
  ...
  //Metoda për të krijuar chunk të dobishme të të dhënave për ndërfaqen e përdoruesit duke përdorur të dhënat e marra nga API
  prepareData():void{
    //Vendosja e vetive të modelit të të dhënave të left container
  ...
  //Metoda për të marrë Detajet e vendndodhjes nga API duke përdorur variablin cityName si Input.
  ...
  getData(){
    ...
    this.getLocationDetails(this.cityName,this.language).subscribe({
      next:(response)=>{
        ...
        //Pasi të marrim vlerat për lattitude dhe longitude gjeografike, mund të thërrasim metodën getWeatherReport.
